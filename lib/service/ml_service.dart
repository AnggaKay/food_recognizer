    ...
    try {
      ...
      _labels = labelsString
          ...
          .map((line) {
            ...
            return ''; // Return empty string for invalid lines
          })
          .where((label) => label.isNotEmpty) // Filter out any empty labels
          .toList();
  ...
  try {
    ...
    for (int i = 0; i < modelInputSize; i++) {
      for (int j = 0; j < modelInputSize; j++) {
        ...
        // If the image has an alpha channel (4 bytes per pixel), skip it.
    ...
    // Assuming a quantized model, create an integer output buffer.
    ...
    // Dequantize the results to get confidence scores.
